...
Versão 3.0 - Com suporte a RELEVO 3D e Visualizador Integrado
"""
...
from src.gui.visualizador_3d import Visualizador3DWidget
...
class MainWindow(QMainWindow):
    ...
    def _criar_painel_resultado(self) -> QWidget:
        """Cria painel de resultados (G-code, estatísticas e visualização 3D)"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        ...
        # Tab: Visualização 3D
        tab_viz = QWidget()
        viz_layout = QVBoxLayout(tab_viz)
        self.visualizador_3d = Visualizador3DWidget()
        viz_layout.addWidget(self.visualizador_3d)
        tabs.addTab(tab_viz, "🎨 Visualização 3D do Relevo")
    ...
    def _gerar_gcode(self):
        ...
        try:
            ...
            # Salva o array para o visualizador
            self.imagem_array = imagem_array
    ...
    def _geracao_concluida(self, gcode, relatorio):
        ...
        # Carrega dados no visualizador 3D
        if hasattr(self, 'visualizador_3d') and self.imagem_array is not None:
            self.visualizador_3d.carregar_dados(
                self.imagem_array,
                self.largura_spin.value(),
                self.altura_spin.value(),
                self.profundidade_spin.value()
            )
        ...
        QMessageBox.information(
            ...
            f"💡 Acesse a aba 'Visualização 3D' para ver o relevo!"
        )
    ...
    def _limpar(self):
        ...
        self.salvar_btn.setEnabled(False)
        self.copiar_btn.setEnabled(False)
        if hasattr(self, 'visualizador_3d'):
            self.visualizador_3d.limpar()
